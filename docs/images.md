...
- Save them directly with the config-setter (no `--image` needed):
  ...
  ```
- Image sessions apply the saved defaults automatically on start; `/aspect <x:y>` and `/format <fmt>` override them for the rest of the session (bare `/aspect`/`/format` show the current value; `clear` unsets it so the parameter is no longer sent — use it to override a saved default for one session).
