...
- **At rest** — encryption covers only the messages sent to the API; the session file (`~/.communicator/sessions/*.json`) stores the transcript in cleartext, and `--e2ee` prints a warning about it on startup (the same caveat `--rpg` prints for its chapter session files, `prompt-log.jsonl` and any legacy `history.json`).
