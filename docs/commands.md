...
On macOS the prompt input also offers spelling assistance: a dictionary completion for the word before the caret appears as a dim hint, and **Tab** accepts it only when no command list is open (otherwise Tab keeps filling the command list). **Ctrl+.** lists spelling replacements for the underlined word under the caret — arrows move, **Enter** applies, **Escape** closes. While a spelling provider is present the prompt footer advertises it as `Ctrl+.: replacements`. Ctrl+. needs a terminal that reports modified keys the way the kitty keyboard protocol (or xterm's modifyOtherKeys) does — macOS Terminal.app reports neither, so on that terminal Ctrl+. does nothing and the footer omits the hint (the underline, the hint and Tab still work).

With `/spelling autocorrect on` (off by default) the character that closes a word — a space or sentence punctuation — replaces a confidently misspelled prose word with the system checker's correction, as one undoable edit (a single `Ctrl+Z` restores what you typed). It never fires on a `/`-command line, only a single-character insert can trigger it (text pasted as one chunk is never rewritten), and anything typed, moved or opened before the result lands (the correction is fetched in the background) simply skips the fix. The same dictionary-gap words that can be underlined (`npm`, `openai`) can also be corrected.

On macOS the prompt input uses the system spell checker: misspelled prose words are underlined as you type (see `/spelling`), while paths, flags and URLs are never flagged — a code-ish word your dictionary does not know (`npm`, `openai`, or the `deepseek` inside `deepseek-chat`) can still be underlined.
