...
| `/settings`     | Show the macOS spelling assistance settings, or set one with `/settings typo\|autocomplete\|autocorrect on\|off` (persisted). macOS only: misspelled prose in the prompt input is underlined while typing, Ctrl+. lists replacements and autocorrect (off by default) fixes a misspelled word when the character that closes it is typed; the line ends with `(inactive on this platform)` elsewhere |
