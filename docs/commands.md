...
`--no-save` is the exception to that persistence: a headless run (a prompt argument or piped stdin) reads, generates and prints exactly as it would otherwise, but leaves nothing behind — no session file (and no empty session claim), no preferences write, and nothing in the `--rpg` directory. It is meant for scripted runs, CI and smoke tests. The preferences a run resolves still shape that run (safe mode, the watermark, the image defaults), they are just not saved. It cannot be combined with an interactive session (which always saves) or with `--list-*`/`--export`/`--delete`/`--delete-all-sessions`, and on a text run `--aspect-ratio`/`--image-format` are rejected too because there they only persist an image default. `--resume <id>` reads the session and answers without rewriting it, and `--image` still writes the generated image files (and `--output-dir` copies) — those are the run's output.
