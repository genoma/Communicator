...
On macOS the prompt input also offers spelling assistance: a dictionary completion for the word before the caret appears as a dim hint, and **Tab** accepts it only when no command list is open (otherwise Tab keeps filling the command list). **Ctrl+.** lists spelling replacements for the underlined word under the caret — arrows move, **Enter** applies, **Escape** closes. Ctrl+. needs a terminal that reports modified keys the way the kitty keyboard protocol (or xterm's modifyOtherKeys) does — macOS Terminal.app reports neither, so on that terminal Ctrl+. does nothing (the underline, the hint and Tab still work).
