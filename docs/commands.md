...
| `/settings`     | Show the macOS spelling assistance settings, or set one with `/settings typo\|autocomplete\|autocorrect on\|off` (persisted). macOS only: misspelled prose in the prompt input is underlined while typing and Ctrl+. lists replacements (autocorrect is stored for a later release), and the line ends with `(inactive on this platform)` elsewhere |
