...
|       | `--web-results`       | `<n>`    | Number of web search results, 1–20 (OpenRouter only, default 10). Implies `auto` mode. A run persists the implied `auto` mode per model; the standing count default is the global `webResults` preference, set with `/web-results <n>` |
...
| `-r`  | `--resume`            | `[partial-id]` | Resume a saved session. No arg = picker (needs a TTY); partial ID = prefix match, unique prefix resumes directly. With `--rpg`, must be bare: continues a story chapter from `<dir>/sessions/` (chapter picker when more than one exists; most recent chapter when piping) |
| `-x`  | `--export`            | `[partial-id]` | Export saved session(s). No arg = multi-select checkbox (needs a TTY); partial ID = prefix match, unique prefix exports directly. Export is best-effort per session; any session it could not export is reported and it exits 1 |
...
|       | `--delete`            | `[partial-id]` | Delete saved session(s) (asks for confirmation on a terminal). No arg = multi-select checkbox (needs a TTY); partial ID = prefix match, unique prefix deletes directly (with piped stdin the printed match is the confirmation). Removal is best-effort per session; anything it could not remove is reported and it exits 1 |
...
|       | `--no-save`           | —        | Run without writing session files or preferences (headless runs). Requires a prompt argument or piped stdin; see the note below the table |
...
Flags shape the run they are given to: **there is no set-a-preference-and-exit form**. With no prompt on a terminal, a flag-bearing invocation opens an interactive chat (`communicator --no-watermark`, `communicator -m <id>`), and the run's exit persists the preferences it resolved (safe mode, the watermark and the image defaults are written at launch instead); the runtime-only display knobs (`--no-smooth-streaming`, `--smooth-speed`, `--compact-thinking`) apply to that session and are persisted by `/smooth` and `/compact-thinking`.
...
`--no-save` is the exception to that persistence: a headless run (a prompt argument or piped stdin) reads, generates and prints exactly as it would otherwise, but leaves the saved session state alone — no session file, no preferences write, and no chapter in the `--rpg` directory; a `--resume <id>` run reads the session and answers without rewriting its file. The session id is still reserved and its 0-byte claim removed, so the sessions directory is created when it did not exist, and no `.index.json` entry is added or changed for this run — a missing or stale index is rebuilt exactly as any listing rebuilds one. It is meant for scripted runs, CI and smoke tests. The preferences a run resolves still shape that run (safe mode, the watermark, the image defaults), they are just not saved. It cannot be combined with an interactive session (which always saves) or with `--list-*`/`--export`/`--delete`/`--delete-all-sessions`, and on a text run `--aspect-ratio`/`--image-format` are rejected too because there they only persist an image default (an image run — `--image` or `-m <image-model>` — still shapes its request with them). What the run was explicitly asked to produce is still written: the generated image files (and `--output-dir` copies), downloaded artifacts (stored as attachment blobs under the sessions dir's `attachments/<session id>/`), the `--debug` prompt log in the `--rpg` directory, and the fill-in templates of a `--rpg` directory that has no story files yet (that launch stops at the setup notice without generating, exactly as it does without `--no-save`).
...
communicator -m "openai/gpt-4o" --attach notes.md "Fix typos:"                    # prompt argument + attachment (piped stdin is read only when no prompt is given)
...
| `/status`      | Print the current settings snapshot (`[thinking]`, `[temp]`, `[top-p]`, `[web]`, `[budget]`, `[smooth]`, plus `[zdr]`/`[e2ee]`/`[compact-thinking]` when active) — re-printed after settings changes (`/markdown` and `/spelling` print only their own confirmation) |
...
| `/web-results` | Set the global web search result count (`/web-results <n>`; only OpenRouter sends it, so on Venice it stores and displays the value without effect), show it with no args |
...
| `/spelling`     | Show the spelling assistance settings, or set one with `/spelling typo\|autocomplete\|autocorrect on\|off` (persisted). Misspelled prose in the prompt input is underlined while typing, Ctrl+. lists replacements and autocorrect (off by default) fixes a misspelled word when the character that closes it is typed. macOS checks through the system spell checker, Linux and Windows through a built-in English dictionary; their listing ends with `(dictionary completions are macOS-only)`, because dictionary completions need the system checker |
...
Image sessions additionally accept `/status` (prints the current settings snapshot — the model and the saved/session sizing badges from the connect banner), `/model` (opens the unified picker: another image model switches the session in place, a text model continues the session as a chat with the same history), `/aspect <x:y>`, `/format <fmt>`, `/resolution <tier>`, `/quality <level>` and `/variants <n>` (bare shows the model's supported values with the current one marked, e.g. `Aspect ratios: 1:1 [16:9] 3:2.`; `clear` unsets it), and `/seed <int>` (session-only, never persisted) to set the aspect ratio, output format, resolution tier, quality level, variant count and random seed for the rest of the session — see [docs/images.md](images.md). `/watermark on|off` and `/safe-mode on|off` (both Venice only) show/hide the Venice watermark and toggle the Venice safe-mode setting on generated images. The sizing commands (`/aspect`, `/format`, `/resolution`, `/quality`, `/variants`) appear only when the active model's constraints advertise the parameter — e.g. `/resolution` is hidden for models without resolution tiers and `/variants` for models that accept at most one image.

While typing at the prompt, a live list of matching commands appears below the input as soon as the line starts with `/` (single line, cursor at line end, not yet an exact match). **Tab** fills the first match, **Shift+Tab** fills the last one, and **Enter** submits (except while the spelling replacement list is open, where Enter applies the selected replacement instead — see below). Typing an exact match hides the list; if Tab/Shift+Tab filled it, the list stays open with that match selected so cycling keeps working. It closes when the line stops matching any command — press Escape, or backspace the `/`, to dismiss it. Parameterized commands like `/temp 0.7` are typed manually after completion.

The prompt input also offers spelling assistance: **Ctrl+.** lists spelling replacements for the underlined word under the caret — arrows move, **Enter** applies, **Escape** closes. On macOS a dictionary completion for the word before the caret appears as a dim hint, and **Tab** accepts it only when no command list is open (otherwise Tab keeps filling the command list); dictionary completions need the system spell checker, so they are macOS-only. While a spelling provider is present the prompt footer advertises it as `Ctrl+.: replacements`. Ctrl+. needs a terminal that reports modified keys the way the kitty keyboard protocol (or xterm's modifyOtherKeys) does — macOS Terminal.app reports neither, so on that terminal Ctrl+. does nothing and the footer omits the hint (the underline, the hint and Tab still work).

With `/spelling autocorrect on` (off by default) the character that closes a word — a space or sentence punctuation — replaces a confidently misspelled prose word with the checker's correction, as one undoable edit (a single `Ctrl+Z` restores what you typed). It never fires on a `/`-command line, only a single-character insert can trigger it (text pasted as one chunk is never rewritten), and anything typed, moved or opened before the result lands (the correction is fetched in the background) simply skips the fix. The same dictionary-gap words that can be underlined (`npm`, `openai`) can also be corrected.

The prompt input checks prose as you type (see `/spelling`): on macOS through the system spell checker and your own dictionaries, on Linux and Windows through a built-in English dictionary. Misspelled prose words are underlined, while paths, flags and URLs are never flagged — a code-ish word the dictionary does not know (`npm`, `openai`, or the `deepseek` inside `deepseek-chat`) can still be underlined.
