...
- `spellingTypoDetection` / `spellingAutocomplete` / `spellingAutocorrect` — macOS-only spelling assistance in the prompt editor (defaults `true` / `true` / `false`): typo detection underlines misspelled prose while you type (Ctrl+. lists replacements for the word under the caret), word autocomplete shows a dim inline hint accepted with Tab while no `/`-command list is open, and autocorrect replaces a confidently misspelled word with the system checker's correction when the character that closes the word (a space or sentence punctuation) is typed — one undoable edit, and only while the caret is still right after it. Set with `/spelling typo|autocomplete|autocorrect on|off` (persisted; the command is listed on macOS only). On other platforms the feature is inert and the stored values are ignored.
