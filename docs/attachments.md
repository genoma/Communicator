...
- **Image preprocessing** — before sending, png/jpg/jpeg/webp attachments are downscaled to a 2048 px long edge and re-encoded with EXIF (including GPS) stripped, when the prebuilt image codec is available; EXIF orientation is baked in first. gif, bmp, animated WebP, undecodable images and runs without the codec pass through byte-for-byte. The `attached:` size and the bytes stored in the session and `--export` are the transformed ones.
- **Limits** — images over 20 MB and pdf/office files over 25 MB are rejected based on the raw file size; text files over 25 MB are rejected too. Inline text over 256 KB is still accepted but warns about context usage.
- **Display & export** — resumed sessions render `attached: <filename> (<kind>)` lines under user messages (replay prints `(file)` for pdf/office where live `/attach` prints `(pdf)`/`(office)`) and `image: <filename>` / `file: <filename>` lines under assistant messages (images show as `image.<ext>` because only the image URL is persisted; pdf/office keep their real filenames); `--export` materializes image/pdf/office attachments as real files under `session-<id>/attachments/` and renders `> **Attachment:** [<filename>](attachments/<filename>)` links for user attachments (assistant-produced parts render `> **Image:**` / `> **File:**`); text-file attachments stay inline in the message text with no filename line; `/copy` copies only the message text, never file contents or base64.
