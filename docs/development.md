...
- The manifest must be canonical **before** the tag: npm rewrites the registry copy of it at publish time and only warns (it normalized `./index.js` to `index.js` on the first publish; the tarball keeps the file we wrote). The workflow's `npm pkg fix` + `git diff --exit-code` step fails the release rather than publishing a manifest npm would rewrite; that same check is worth running by hand after editing `bin`, `repository` or `publishConfig`.
...
- Clipboard probe order: macOS `pbcopy`, Windows `clip`, Linux `wl-copy` → `xclip` → `xsel`, with a 10 s timeout per tool. Clipboard image reads (`/paste`) probe macOS `osascript` first for a `clipboard info`-gated `«class furl»` whose POSIX path comes back on stdout (a file URL is preferred, so a Finder copy keeps its real name; a screenshot copied straight to the clipboard offers only image flavours), then the `«class PNGf»` write, then the `«class TIFF»` fallback, Windows PowerShell 5.1 `Get-Clipboard -Format Image`, Linux `wl-paste --type image/png` → `xclip -selection clipboard -t image/png -o`, with the same per-tool timeout; `COMMUNICATOR_CLIPBOARD_TEST=1` opts `test/clipboard-real.test.js` into the real round trip, which clobbers the pasteboard (CI sets it on the macOS leg only) (`src/clipboard.js`).
