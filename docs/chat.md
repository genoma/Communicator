...
- A truncated answer is marked: `Output limit reached — the answer above is incomplete.` after the answer, both live and when the session is resumed, and the markdown export carries a `**Note:**` line. An answer that ended with a provider error or a content filter after producing text says so with the same live/replay notice (the markdown export note is output-limit-specific).
...
The same line is re-printed as `Current settings: …` after every mid-chat config change (`/temp`, `/top-p`, `/reasoning`, `/web-search`, `/web-results`, `/smooth`, `/budget`, `/export-format`, `/model`, `/new`) and by `/status` on demand. All values are persisted per model where applicable, so the next session's banner reflects what you last set.
...
The pace is a global speed setting, persisted in preferences under `smoothSpeed` (shared by every session, like `smoothStreaming`). `/smooth slow|normal|fast|<cps>` persists it (and enables smooth streaming); `--smooth-speed <level|cps>` at launch (`slow` ≈ 500 chars/s, `normal` ≈ 2000 chars/s, `fast` ≈ 8000 chars/s, or any positive chars-per-second value) applies to that run only. Speed changes apply live: the very next tick of an in-flight stream uses the new pace. The speed is inert when smooth streaming is off or output is piped.

While the model is working, a dim indicator appears on the response line roughly 200 ms after you send the message — `Waiting for response · 2.3s` with a braille spinner, the elapsed wait ticking live (or `Searching the web` when web search is forced with `always`, since that mode is guaranteed to search). The live clock is the burst-mode mitigation: an endpoint that delivers the whole answer in one burst (the web-search path does) leaves a silent gap, and the seconds show the real wait rather than a frozen spinner. The indicator is erased the moment the first token arrives, and it never shows for instant replies. When the stream ends without reasoning, the indicator line resolves to a green `✓ Waiting for response` checkpoint (label only, no seconds), followed by a blank row before the answer, and it is kept in the transcript and replayed on history re-renders (`--resume`, terminal resizes) just like the compact-thinking checkpoint. With compact thinking on, the indicator doubles as the meter: same live wait clock, then it switches to the thinking counter (`Thinking · 1.2k · 3.5s`) when reasoning starts.
...
The files are combined into one fixed system message; the scenario is placed after the character and user sections, just before the per-turn rules. `{{char}}` and `{{user}}` are replaced with the names from the first `# Name` heading in `char.md` and `user.md`; Markdown comments are removed before the prompt is sent. Both `char.md` and `user.md` must keep a real `# <name>` heading as their first H1 — it is the speaker name shown in the transcript (`❯ <name>`) and the value that `{{char}}`/`{{user}}` stands for, and a missing or placeholder heading stops the run with an error telling you the convention. `--rpg` cannot be combined with `--system-prompt` or a session-id `--resume`, and is for text chat models only. A bare `-r`/`--resume` next to `--rpg` means "continue the story" — it resumes a saved chapter, falling back to the legacy dir-local `history.json` only when no chapter session exists (see below).

If `post-history-instruction.md` contains anything (Markdown comments stripped, `{{char}}`/`{{user}}` expanded), it is sent as a separate system message after the latest user message on every turn. The file is optional — a missing or empty file disables the feature. This is the SillyTavern "post-history instructions" pattern: late instructions get the model's strongest attention and are not diluted by a long conversation, so it is the place for rules the model tends to forget or skim over (never break character, always react to X, keep replies under N words, …). The instruction is added at request time and never saved into the chapter session.
...
RPG transcripts use named speaker markers: replayed user turns are shown under `❯ <your name>` (instead of the usual `❯ You`) and the character's turns under `❯ <character name>`, both taken from the `# Name` headings in `user.md` and `char.md`. In RPG mode the character marker replaces the `❯ Answer` label entirely — the character's turn is labelled by its speaker alone, so a turn never shows both (this follows SillyTavern's card-per-character style). Live replies (including one-shot TTY runs) stream under the character marker with the same one-blank-above-and-below spacing. Non-RPG chat is unchanged.
