...
**Legacy stories:** the old dir-local `history.json` log is legacy — it is no longer written by any run. On the first run against a story directory that only holds a `history.json`, the saved story is imported once as a chapter session (`Migrated the saved story (N messages) into <dir>/sessions/<id>.json…`) and everything continues as a chapter; the file itself is left untouched. If no model can be resolved for the import (no `-m` and no saved preference) the import is skipped and the legacy `history.json` fallback keeps working. Support for this legacy layout is temporary and will be removed in a future release.
