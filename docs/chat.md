...
A plain `--rpg <dir>` run always starts a **new** story: the greeting from `first-message.md` opens the chat, and once the new story has turns it replaces `history.json` (a notice first counts the earlier sessions available — `Starting a new story in <dir> (N earlier session(s) available…)`). Resuming is opt-in: `communicator --rpg <dir> --resume` (bare `--resume`, no session id) continues a chapter — the only chapter resumes directly, and with more than one the same picker a normal `-r` uses lets you choose, printing `Resumed RPG conversation from <dir>/sessions/<id>.json (N messages).` (piped one-shots fall back to the most recent chapter; a `history.json` notice appears only for a story saved before the chapter layout); if there is nothing to resume it simply starts a new story. The resumed chapter restores its session settings — model, provider, temperature, `top-p`, reasoning effort, budget and web-search flags — with the run's flags (`-m`, `--temperature`, …) taking precedence, and keeps writing to its own chapter file. An e2ee mismatch is refused, exactly like a normal `-r` (an e2ee chapter needs `--e2ee` and vice versa). Each run's chapter is also written as a session (same `*.json` + `.index.json` + `attachments/` layout as `~/.communicator/sessions/`) under `<dir>/sessions/`, so chapters are self-contained in the RPG folder and the global sessions directory holds only ordinary chats.
