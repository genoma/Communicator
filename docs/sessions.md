...
- Assistant messages may carry `finishReason` when the provider ended the turn abnormally (anything but `stop`, e.g. `length`, `content_filter`, `error`). It drives the truncation/early-end notice on resume, and the markdown export note for a `length` answer that produced text; a `length` finish also means the stored answer is incomplete. Older sessions without the field render as complete replies
