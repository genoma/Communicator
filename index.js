...
program
  ...
  .option('--list-image-models', 'list available image generation models and exit (Venice only)')
  .option('--image', 'generate an image with a Venice image model and exit')
  .option('--image-model <id>', 'image generation model ID (skips the interactive image model picker)')
  .option('--image-format <png|jpeg|webp>', 'image output format (default: webp)')
  .option('--variants <n>', 'number of images to generate (1-4)')
  .option('--aspect-ratio <x:y>', 'image aspect ratio, model-dependent (e.g. 16:9)')
