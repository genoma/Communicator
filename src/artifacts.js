...
export async function produceParts(streamedParts, { sessionId, imageOutputSupported, fullText, requestFn, sessionsDir = null, signal }) {
  ...
  const results = await mapWithConcurrency(parts, ARTIFACT_DOWNLOAD_CONCURRENCY, async (part) => {
    ...
    const res = await downloadRemotePart(part, sessionId, { requestFn, sessionsDir, signal })
    return { ...res, label }
  })
...
export async function resolveArtifacts(apiResult, { sessionId, imageOutputSupported, requestFn, sessionsDir = null, signal }) {
