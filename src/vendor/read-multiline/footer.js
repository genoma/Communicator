...
const KITTY_REQUIRED_KEYS = new Set([
    "shift+enter",
    "ctrl+enter",
    "cmd+enter",
]);
...
function getAvailableModifiedKeys(disabledKeys) {
    return MODIFIED_KEY_ORDER.filter((k) => {
        if (disabledKeys.has(k))
            return false;
        if (_kittySupported === false && KITTY_REQUIRED_KEYS.has(k))
            return false;
        return true;
    });
}
function resolveAction(action, preferNewlineOnEnter, submitLabels, newlineLabels) {
    switch (action) {
        case "submit":
            if (!preferNewlineOnEnter)
                return { keys: ["Enter"], action: "submit" };
            return submitLabels.length > 0 ? { keys: submitLabels, action: "submit" } : null;
        case "newline":
            if (preferNewlineOnEnter)
                return { keys: ["Enter", ...newlineLabels], action: "newline" };
