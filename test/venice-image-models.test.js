...
test('fetchImageModels requests ?type=image and normalizes constraints', async (t) => {
  ...
  assert.equal(m.supportsStyleReferences, true)
  ...
  assert.equal(m.constraints.defaultResolution, '1K')
  assert.deepEqual(m.constraints.qualities, ['low', 'medium', 'high'])
  assert.equal(m.constraints.defaultQuality, 'high')
  assert.equal(m.constraints.promptCharacterLimit, 2048)
  assert.deepEqual(m.constraints.steps, { default: 25, max: 50 })
  assert.equal(m.constraints.widthHeightDivisor, 8)
  assert.equal(m.constraints.maxStyleReferences, 3)
...
test('fetchImageModels falls back to the id for the name and defaults optional fields', async (t) => {
  ...
  assert.equal(models[0].supportsStyleReferences, false)
