...
- The same word-aware folding covers the streamed reasoning body (`writeSegment('reasoning')`), the plain (non-markdown) content path and history replay of both: `createWordWrap` (`src/ui/wrap.js`) emits pieces as they arrive while holding the current word until its fit is known — lines fold at spaces and over-long words stream in exact-width chunks; every text piece goes through the given style (`dim` for reasoning) while fold/newline bytes stay raw. Without a usable width it passes text through unchanged (byte-identical pipes). Instance lifecycle: `end_reasoning` and `render.flush()` flush the wraps; the compact-thinking path is untouched (meter owns the line).
