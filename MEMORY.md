...
- **Esc stop finalization** (`createTurnRunner` stopped path): on a TTY stop the runner flushes the smooth renderer synchronously, writes `\n\n` (one blank row), a dim `Stopped` row, and a trailing `\n` (blank row below) before the loop's separator, so the stopped note has a blank row above and below it and the next prompt follows the loop sep. The partial is appended as the turn result (persisted); a stopped turn with no delivered output shows only the dim `Stopped` line and returns to the prompt — no exit. The `Stopped` line is live-only: history replay / resize rebuilds show the persisted partial assistant message, never the stop note.
