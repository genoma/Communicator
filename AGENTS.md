...
- Run `npm test` (via `scripts/run-tests.js`, which wraps `node --test --experimental-test-module-mocks` with `NO_COLOR` set, `FORCE_COLOR` cleared — the raw `node --test` invocation is not color-deterministic and fails ~55 ANSI-contract tests on a TTY — `OPENROUTER_API_KEY`/`VENICE_API_KEY` cleared (the suite is key-free like CI; a test that needs a key stubs it) and `HOME`/`USERPROFILE` pointed at a throwaway directory, so an unhomed test file cannot read or write the developer's real `~/.communicator.json`; those guarantees hold only for `npm test`, not for a bare `node --test`) after any change. The wrapper also loads the test-child console guard (`--import scripts/test-child-console-guard.js`) that keeps app output off the runner's frame channel: a bare `node --test` bypasses it and can flake with `Unable to deserialize cloned data due to invalid or unsupported version.` (upstream nodejs/node#62693).
