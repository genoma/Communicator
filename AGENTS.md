...
- The `engines` field is the minimum actually required by current code + tests: 22.3 is pinned by `--experimental-test-module-mocks` (used for `mock.module()` in tests), not by runtime dependencies. Only bump it when a new dependency or a new Node API/flag actually requires a newer version; never because the installed homebrew Node is newer.
...
- WIP, do not merge yet: the frame-diffing editor (in-repo `src/editor/` replacing the vendored read-multiline editor) lives on branch `feat/editor-bufferdiff-v2`; Implementation facts live in MEMORY.md §Command autocomplete.
- The vendored `@toiroakr/read-multiline` editor (`src/vendor/`) is no longer used by app code; keep it as reference/rollback.
