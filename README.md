...
- **Per-session budget caps** — `--budget <usd>` or `/budget <usd>` limits accumulated session cost; warns at 80% used and refuses further turns at 100% in interactive sessions (one-shot runs never pre-check the cap)
