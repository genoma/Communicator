...
- **RPG mode** — `--rpg <dir>` assembles `char.md`, `user.md`, `prompt.md`, and `scenario.md` into one fixed roleplay system prompt and seeds `first-message.md` as the opening assistant turn; missing files are created as fill-in templates. Add `--debug` to log every request to `prompt-log.jsonl` in the directory. See [docs/chat.md](docs/chat.md)
