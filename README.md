...
- **Image generation (Venice)** — generate images directly from the CLI with `--image` (one-shot, `--list-image-models` to browse models, per-model sizing flags), `/image <description>` inside a Venice chat, or by picking an image model in the unified model picker, which starts a dedicated image session where every prompt generates an image (`/help`, `/exit`, `/quit`; resumable). Image model ids also work with `-m <image-model> "prompt"` for one-shot generation. Images are saved into the session, resumable and exportable, with an optional `--output-dir` copy. Disable the Venice watermark on generated images with `--no-watermark` (persisted as a global setting) or `/watermark off` in a Venice session
...
| [`docs/images.md`](docs/images.md) | Venice image generation: unified model picker, image sessions, `--image` flags, `/image`, `-m` parity, model listing, sizing, storage |
