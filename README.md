...
- **Slash commands** — `/new`, `/model`, `/reasoning`, `/temp`, `/top-p`, `/budget`, `/web-search`, `/attach`, `/retry`, `/edit`, `/copy`, `/markdown`, `/smooth`, `/compact-thinking`, `/cost`, `/quit`, and more (image sessions add sizing commands plus Venice-only `/watermark`). See [docs/commands.md#slash-commands](docs/commands.md#slash-commands)
