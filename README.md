...
- **Idle hint** — on a TTY, once the answer has started, a quiet gap (e.g. a web search after the first line) shows a dim animated `. .. ...` suffix so the turn never looks dead; it is erased on the next token and never persisted or replayed
...
- **Spelling assistance (macOS only)** — misspelled prose in the prompt input is underlined as you type, using the system spell checker and your own dictionaries (a small helper Communicator compiles on first use into `~/.communicator`, falling back to a per-call `osascript` process whenever the build is not available — never the other way around, and a Mac without Command Line Tools never compiles anything at all: the compiler shim is not invoked, so no installer is ever offered); paths, flags and URLs are never flagged, but a code-ish word your dictionary does not know (`npm`, `openai`, or the `deepseek` inside `deepseek-chat`) can still be underlined. Ctrl+. lists replacements for the flagged word under the caret, and a dictionary completion appears as a dim hint accepted with Tab while no `/`-command list is open. Autocorrect is off by default: with `/settings autocorrect on`, the character that closes a word (a space or sentence punctuation) replaces a confidently misspelled word with the checker's correction as one undoable edit — only a single-character insert triggers it, so pasted text is never rewritten, and the same dictionary-gap words that can be underlined can also be corrected. Toggle with `/settings typo|autocomplete|autocorrect on|off` (persisted). Inert on other platforms
...
- **Slash commands** — `/new`, `/model`, `/reasoning`, `/temp`, `/top-p`, `/budget`, `/web-search`, `/attach`, `/retry`, `/edit`, `/delete`, `/copy`, `/markdown`, `/smooth`, `/compact-thinking`, `/settings`, `/cost`, `/quit`, and more (image sessions add sizing commands plus Venice-only `/watermark`). See [docs/commands.md#slash-commands](docs/commands.md#slash-commands)
