...
- **Image generation** — generate images directly from the CLI on **both providers** with `--image` (one-shot, `--list-image-models` to browse models, per-model sizing flags), `/image [--ratio <x:y>] [--format <fmt>] <description>` inside a chat, or by picking an image model in the unified model picker, which starts a dedicated image session where every prompt generates an image (`/help`, `/model`, `/exit`, `/quit`, `/aspect`, `/format`; resumable). `/model` inside the image session switches image models or picks a text model to continue the session as a normal chat. Image model ids also work with `-m <image-model> "prompt"` for one-shot generation. Aspect ratio and output format choices are remembered as per-provider defaults (saved via `--aspect-ratio`/`--image-format` alone, pickers with preselected defaults, or `/aspect`/`/format` in image sessions); unsupported saved defaults are dropped with a visible note instead of being sent blindly. Images are saved into the session, resumable and exportable, with an optional `--output-dir` copy. Disable the Venice watermark on generated images with `--no-watermark` (persisted as a global setting) or `/watermark off` in a Venice session; disable Venice safe mode with `--no-safe-mode` (also persisted as a global setting)
...
| [`docs/images.md`](docs/images.md) | Image generation on Venice and OpenRouter: unified model picker, image sessions, `--image` flags, `/image`, `/aspect`/`/format`, `-m` parity, model listing, sizing defaults, storage |
